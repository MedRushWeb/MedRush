...
for (let idx = 0; idx < Final_Count; idx++) {
    if (submitted_string[`submitted_string_${test_result_index}_${uid}`][idx] === true) {
        Answers[`Answer_${test_result_index}_${uid}`] = chosen_Answers;

        if (FINAL_ARRAY[idx].right_answer === FINAL_ARRAY[idx][`answer_${Answers[`Answer_${test_result_index}_${uid}`][idx + 1] + 1}`]) {
...
for(let i=1;i<=Final_Count;i++){
   if(submitted_string[`submitted_string_${test_result_index}_${uid}`][i-1]){
...
icon_visibility.push(test_result_index);
...
// this line i made to stop depending on a local variables that will be changed 
// and initialized a lot of time and cause data to disappear
if((FINAL_ARRAY[i].right_answer)==FINAL_ARRAY[i][`answer_${Answers[`Answer_${test_result_index}_${uid}`][i+1]+1}`]){
...
// i<n
// here i changed n to finalcount
for ( let i=0 ; i<Final_Count; i++){
...
// hhaaaii ghalat z laze, exam_index_number
if (submitted_string[`submitted_string_${test_result_index}_${uid}`][i] == true){
...
console.log("submitted_array33",submitted_array);
  ...
  markedIdsArrayCloud=Array.from(new Set(all_marked.map(q => q.id)));
  examIdsArray[`examIdsArray_${test_result_index}_${uid}`] = extractExamIds(test_result_index, uid, Exams);

  // Clean time and submitted arrays
  const rawTimeArray = timeperQuestion[`timeperQuestion_${test_result_index}_${uid}`] || [];
...
const rawStatusArray = status_string[`status_string_${test_result_index}_${uid}`] || [];
...
rawStatusArray[i] === undefined ? "" : rawStatusArray[i]  // or whatever default value you want
  ...
  const selectedData = {
    markedIdsArrayCloud,
    [`examIdsArray_${test_result_index}_${uid}`]: examIdsArray[`examIdsArray_${test_result_index}_${uid}`],
    [`Answer_${test_result_index}_${uid}`]: Answers[`Answer_${test_result_index}_${uid}`],
    [`Date_${test_result_index}_${uid}`]: Dates[`Date_${test_result_index}_${uid}`],
    [`score_${test_result_index}_${uid}`]: scores[`score_${test_result_index}_${uid}`],
    [`status_string_${test_result_index}_${uid}`]: cleanedStatusArray,
    [`submitted_string_${test_result_index}_${uid}`]: cleanedSubmittedArray,
    [`timeperQuestion_${test_result_index}_${uid}`]: cleanedTimeArray,
  ...
  console.log("submitted_array44",submitted_array);
