  ...
  function normalizePeriodFields(data, fallbackMonths = 12) {
    const now = new Date();
    const approvedAt =
      data?.approvedAt ||
      data?.createdAt ||
          ...
          eventHandler: async (event) => {
            if (event.event === "Checkout.Success") {
              ...
              try {
                ...
                // Normalize dates (fallback = +12 month if server didn't include period end)
                const { approvedAt, currentPeriodEnd } = normalizePeriodFields(data, 12);
  ...
  async function checkSubscription() {
    ...
    // Normalize any dates from server, fallback to +12 month from now if missing and ACTIVE
    ...
    if (!currentPeriodEnd && status === "active" || status === "ACTIVE") {
      const tmp = addMonths(new Date(), 12);
